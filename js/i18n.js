...
const STRINGS = {
  tw: {
    ...
    helpTsT: '時間序列模式',
    helpTsD: '把每次定位結果留在地圖上，看得出目標的移動。每台裝置最多保留 20 點，超過時擠掉自己最舊的。停止調整達「更新間隔」後這次定位才算完成，在那之前微調只會移動同一個點。「分享追蹤目標」產生的連結只含記錄點，對方開啟後併入他的地圖，不會覆蓋他原有的點或設定。',
  ...
  en: {
    ...
    secShort: '{n}s',
    minShort: '{n}m',
    pinClearTitle: 'Clear the recorded points',
    pinClearOk: 'Clear all',
    ...
    helpTsT: 'Time series',
    helpTsD: 'Keeps every fix on the map so a movement is visible. Each device keeps its latest 20 points, dropping its own oldest beyond that. A fix is committed once it has sat unchanged for the chosen interval; until then, tuning moves the same point. Share track makes a link holding only the recorded points: opening it merges them into the other map without replacing their points or settings.',
    helpEstT: 'Centroid / MLE',
